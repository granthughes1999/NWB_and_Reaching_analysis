...
def trial_by_trial(spike_times, event_times, pre, post, bin_size, brain_region=None):
    ...
    if event_times.size == 0:
        bytrial = np.zeros((0, n_bins), dtype=float)
        psth = np.zeros(n_bins, dtype=float)
        var = np.zeros(n_bins, dtype=float)
        return psth, var, centers, bytrial

    bytrial = np.zeros((len(event_times), n_bins), dtype=float)
    ...
    var = np.nanstd(bytrial, axis=0) / bin_size / np.sqrt(max(len(event_times), 1))
    return psth, var, centers, bytrial
...
def _heatmap_rows(df_probe, unit_ids, event_times, pre, post, bin_size, normalize_fr=False, max_fr=60, smoothing_sigma=None):
    ...
    for unit_id in unit_ids:
        ...
        heat = np.sum(bytrial, axis=0)
        if smoothing_sigma is not None:
            heat = smooth_data(heat, sigma=smoothing_sigma)
        if normalize_fr:
            mean_fr = np.mean(heat)
            std_fr = np.std(heat)
            heat = (heat - mean_fr) / std_fr if std_fr != 0 else np.zeros_like(heat)
        else:
            heat = np.clip(heat, 0, max_fr)
        rows.append(heat)
        labels.append(int(unit_id))
    if not rows:
        return np.empty((0, 0)), np.array([], dtype=int), np.array([], dtype=float)
    return np.asarray(rows), np.asarray(labels), centers
...
def _multi_region_raster_psth_common(df_units, event_times, brain_regions, probe_units, probe_letters, label, KSlabel, pre, post, binSizeRaster, binSizePSTH, save_dir, highlight_time_zero, normalize_psth, dot_size, background_colors, title):
    ...
    if psth_ax.lines:
        _legend_outside(
            psth_ax,
            [mpatches.Patch(color=line.get_color(), label=line.get_label()) for line in psth_ax.lines],
        )
    axs[-1].set_xlabel('Time (s)')
    _tight_layout_figure(fig, top=0.95, reserve_right=True)
...
def probe_units_heatmap(df_units, df_stim, probeLetter, selected_units=None, pre=0.5, post=1, bin_size=0.05, epoch1='pellet_detected_timestamp', label=False, KSlabel=True, save_dir=None, max_fr=60, show_unit_labels=True, normalize_fr=False, event_times=None):
    ...
    fig.tight_layout()
    return fig, ax, _save_figure(fig, save_dir, f'probe_{probeLetter}_heatmap.png')
...
def sorted_heatmap(df_units, df_stim, probeLetter, selected_units=None, pre=0.5, post=1, bin_size=0.05, epoch1='pellet_detected_timestamp', label=False, KSlabel=True, save_dir=None, max_fr=60, show_unit_labels=True, normalize_fr=True, event_times=None):
    ...
    ax.set_xlabel('Time (s)')
    fig.tight_layout()
    return fig, ax, _save_figure(fig, save_dir, f'probe_{probeLetter}_sorted_heatmap.png')
...
def multi_probe_units_heatmap(df_units, df_stim, probes, selected_units, times_of_events=None, event_name=None, brain_regions=None, pre=0.5, post=1, bin_size=0.01, epoch1='pellet_detected_timestamp', label=False, KSlabel=True, save_dir=None, max_fr=60, show_unit_labels=True, normalize_fr=False):
    ...
    return fig, axes, _save_figure(fig, save_dir, f'multi_probe_heatmap_{event_name or epoch1}_{abs(pre)}_{abs(post)}.png')
...
def multi_probe_units_heatmap_smoothed(df_units, df_stim, probes, selected_units=None, selected_units_by_subplot=None, times_of_events=None, event_name=None, brain_regions=None, pre=0.5, post=1, bin_size=0.01, epoch1='pellet_detected_timestamp', label=False, KSlabel=True, save_dir=None, max_fr=60, show_unit_labels=True, normalize_fr=False, sort_by_time=True, reset_unit_count=False, smoothing_window=1):
    ...
    fig.suptitle(f'Aligned to {event_name or epoch1}')
    fig.tight_layout(rect=[0, 0, 1, 0.97])
    return fig, axes, _save_figure(fig, save_dir, f'multi_probe_heatmap_smoothed_{event_name or epoch1}_{abs(pre)}_{abs(post)}.png')
